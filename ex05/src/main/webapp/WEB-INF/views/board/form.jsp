<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt"  prefix="fmt"%>
<!DOCTYPE html>
<html>

<head>
	<meta charset="UTF-8">
	<c:choose>
		<c:when test="${empty boardInfo }">
			<title>게시글 등록</title>
		</c:when>
		<c:otherwise>
			<title>게시글 수정</title>
		</c:otherwise>
	</c:choose>
</head>

<body>
	<form name="infoForm" action="boardInsert" method="POST">
		<table border="1">
			<tr>
				<th>번호</th>
				<td><input type="text" name="bno" value="${empty boardInfo ? '' : boardInfo.bno }" readonly></td>
			</tr>
			<tr>
				<th>제목</th>
				<td><input type="text" name="title" value="${empty boardInfo ? '' : boardInfo.title }"></td>
			</tr>
			<tr>
				<th>작성자</th>
				<td><input type="text" name="writer" value="${empty boardInfo ? '' : boardInfo.writer }"></td>
			</tr>
			<tr>
				<th>내용</th>
				<td><textarea name="content">${empty boardInfo ? '' : boardInfo.content }</textarea></td>
			</tr>
			<tr>
				<th>작성일자</th>
				<td><input type="date" name="regdate" value="<fmt:formatDate value='${empty boardInfo ? "" : boardInfo.regdate }' pattern="yyyy-MM-dd"/>"></td>
			</tr>
		</table>
		<button type="submit">저장</button>
		<button type="button" onclick="location.href='boardList'">목록</button>
	</form>
	<script>
		infoForm.addEventListener('submit', function (e) {
			e.preventDefault();
			if(${!empty boardInfo}){
				infoForm.action = "boardUpdate";
			}
			infoForm.submit();
		})
	</script>
</body>

</html>